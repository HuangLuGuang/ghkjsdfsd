...
layui.define(["jquery", "laytpl"], function (exports) {
  ...
  var TPL_ELEM = function (options, floor, parentStatus) {
    return [
      ...
      '<div class="eleTree-node {{# if(d[i].visible===false){ }}eleTree-search-hide{{# } }}" data-padding="' +
        options.indent * floor +
        '" data-' +
        options.customKey +
        '="{{d[i]["' +
        options.request.key +
      ...
      (function () {
        ...
        if (floor !== 0) {
          s =
            '<i class="eleTree-node-verticalline" style="left: ' +
            (9 + options.indent * (floor - 1)) +
            'px;"></i>' +
            '<i class="eleTree-node-horizontalline" style="width: ' +
            (options.indent - 4) +
            "px;left: " +
            (9 + options.indent * (floor - 1)) +
            'px;"></i>';
      ...
      '<div class="eleTree-node-content" style="padding-left: ' +
        options.indent * floor +
        'px;">',
        ...
        if (options.showCheckbox) {
          ...
          return [
            ...
            '<input type="checkbox" name="eleTree-node" lay-ignore eleTree-status=' +
              status +
              ' class="layui-hide eleTree-hideen ',
            "{{# } }}",
      ...
      '<span class="eleTree-node-content-label">{{d[i]["' +
  ...
  Class.prototype = {
    ...
    render: function () {
      ...
      options.elem =
        typeof options.elem === "string" ? $(options.elem) : options.elem;
    ...
    renderData: function () {
      ...
      if (!options.lazy) {
        if (
          !options.renderAfterExpand ||
          options.defaultExpandAll ||
          options.defaultExpandedKeys.length > 0 ||
          options.defaultCheckedKeys.length > 0
        ) {
    ...
    reload: function (options) {
      // var _self=this;
      if (this.config.data && this.config.data.constructor === Array)
        this.config.data = [];
      this.config = $.extend({}, this.config, options);
      // $(this.config.elem).off();  // 取消事件绑定，防止多次绑定事件
      // reload记录选中的数据
    ...
    eleTreeEvent: function () {
      ...
      var options = this.config;
      // 下拉
      var expandOnClickNode = options.expandOnClickNode
        ? ".eleTree-node-content"
        : ".eleTree-node-content>.eleTree-node-content-icon";
      options.elem.on("click", expandOnClickNode, function (e) {
        e.stopPropagation();
        var eleTreeNodeContent =
          $(this).parent(".eleTree-node").length === 0
            ? $(this).parent(".eleTree-node-content")
            : $(this);
        var eleNode = eleTreeNodeContent.parent(".eleTree-node");
        var sibNode = eleTreeNodeContent.siblings(".eleTree-node-group");
        var el = eleTreeNodeContent
          .children(".eleTree-node-content-icon")
          .children(".layui-icon");
          ...
          if (options.lazy && el.hasClass("lazy-icon")) {
            ...
            options.load(d, function (getData) {
              ...
              if (d[options.request.children]) {
                d[options.request.children] = d[
                  options.request.children
                ].concat(getData);
              } else {
                d[options.request.children] = getData;
              }
              var eletreeStatus = eleTreeNodeContent
                .children("input.eleTree-hideen")
                .attr("eletree-status");
              if (
                d[options.request.children] &&
                d[options.request.children].length > 0
              ) {
                // 只渲染获取到的数据
                laytpl(TPL_ELEM(options, floor, eletreeStatus)).render(
                  getData,
                  function (string) {
              ...
              el.removeClass(
                "lazy-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"
              ).addClass("layui-icon-triangle-r icon-rotate");
        ...
        if (options.accordion) {
          var node = eleTreeNodeContent
            .parent(".eleTree-node")
            .siblings(".eleTree-node");
    ...
    checkboxEvent: function () {
      var options = this.config;
      var _self = this;
      var checkOnClickNode = options.checkOnClickNode
        ? ".eleTree-node-content"
        : ".eleTree-checkbox";
      ...
      options.elem.on("click", checkOnClickNode, function (e) {
        ...
        var childNode = eleTreeNodeContent
          .siblings(".eleTree-node-group")
          .find("input[name='eleTree-node']");

        // 添加active背景
        if (_self.prevClickEle)
          _self.prevClickEle.removeClass("eleTree-node-content-active");
        if (options.highlightCurrent)
          eleTreeNodeContent.addClass("eleTree-node-content-active");
    ...
    checkboxInit: function () {
      ...
      options.elem.find("input[data-checked]").each(function (index, item) {
        ...
        checkboxEl
          ...
          .removeClass("eleTree-checkbox-line");
        if (options.checkStrictly) return;
        // 选择子孙
        childNode.prop("checked", "checked").attr("eleTree-status", "1");
        childNode
          .siblings(".eleTree-checkbox")
          .addClass("eleTree-checkbox-checked");
        childNode
          .siblings(".eleTree-checkbox")
          .children("i")
          .addClass("layui-icon-ok")
    ...
    demo_chachong: function (siblingNode) {
      siblingNode.each((index, item) => {
        var siblingIsChecked = $(item)
          .children(".eleTree-node-content")
          .children("input[name='eleTree-node']")
          .attr("eleTree-status");
    ...
    selectParents: function (inp, eleNode, siblingNode) {
      ...
      while (Number(eleNode.attr("eletree-floor")) !== 0) {
        ...
        //     })
        // 父元素的实际input
        var parentInput = eleNode
          .parent(".eleTree-node-group")
          .siblings(".eleTree-node-content")
          .children("input[name='eleTree-node']");
        // 父元素的checkbox替代
        ...
        var parentNode = eleNode.parents(
          "[eletree-floor='" +
            (Number(eleNode.attr("eletree-floor")) - 1) +
            "']"
        );
        var parentCheckbox = parentNode
          .children(".eleTree-node-content")
          .children("input[name='eleTree-node']")
          .get(0);
    ...
    initialExpandAll: function (data, arr, floor, isMethodsExpandAll) {
      ...
      data.forEach(function (val, index) {
        arr.push(index);
        if (
          val[options.request.children] &&
          val[options.request.children].length > 0
        ) {
          var el = options.elem
            .children(".eleTree-node")
            .eq(arr[0])
            .children(".eleTree-node-group");
          ...
          laytpl(TPL_ELEM(options, floor)).render(
            val[options.request.children],
            function (string) {
              el.append(string);
              // 判断是否展开所有
    ...
    defaultChecked: function (dataChecked) {
      ...
      arr.forEach(function (val, index) {
        var nodeContent = options.elem
          .find("[data-" + options.customKey + "='" + val + "']")
          .children(".eleTree-node-content");
    ...
    checkboxRender: function () {
      ...
      options.elem
        .find("input.eleTree-hideen[type=checkbox]")
        .each(function (index, item) {
          if ($(item).hasClass("eleTree-disabled")) {
            $(item).after(
              '<div class="eleTree-checkbox eleTree-checkbox-disabled"><i class="layui-icon"></i></div>'
            );
          } else {
            $(item).after(
              '<div class="eleTree-checkbox"><i class="layui-icon"></i></div>'
            );
          }
          ...
          if ($(item).attr("eletree-status") === "1") {
            ...
            checkbox
              .children("i")
              .removeClass("layui-icon-ok eleTree-checkbox-line");
          } else if ($(item).attr("eletree-status") === "2") {
            checkbox.addClass("eleTree-checkbox-checked");
            checkbox
              .children("i")
              .removeClass("layui-icon-ok")
              .addClass("eleTree-checkbox-line");
          }
        });
    },
    // 通过dom节点找对应数据
    reInitData: function (node) {
      ...
      for (var j = 1; j < arr.length; j++) {
        d = d[options.request.children]
          ? d[options.request.children][arr[j]]
          : d;
      }
      for (var k = 1; k < arr.length - 1; k++) {
        parentData = parentData[options.request.children]
          ? parentData[options.request.children][arr[k]]
          : parentData;
      }
      ...
      var fn = function (data) {
        ...
        for (; obj.i < obj.len; obj.i++) {
          if (data[obj.i][options.request.key] != key) {
            if (
              data[obj.i][options.request.children] &&
              data[obj.i][options.request.children].length > 0
            ) {
    ...
    updateKeyChildren: function (key, data) {
      var options = this.config;
      var node = options.elem.find(
        "[data-" + options.customKey + "='" + key + "']"
      );
      ...
      this.keySearchToOpera(key, function (d, obj) {
        ...
        // dom更新
        node.length !== 0 &&
          laytpl(TPL_ELEM(options, floor)).render(data, function (string) {
            $(node).children(".eleTree-node-group").empty().append(string);
            options.defaultExpandAll &&
              $(node).children(".eleTree-node-group").show();
          });
    ...
    updateKeySelf: function (key, data) {
      ...
      var node = options.elem
        .find("[data-" + options.customKey + "='" + key + "']")
        .children(".eleTree-node-content");
      // var floor = Number(node.attr("eletree-floor")) + 1;
      data[options.request.name] &&
        node
          .children(".eleTree-node-content-label")
          .text(data[options.request.name]);
      data[options.request.disabled] &&
        node
          .children(".eleTree-hideen")
          .addClass("eleTree-disabled")
          .siblings(".eleTree-checkbox")
    ...
    append: function (key, data) {
      ...
      this.keySearchToOpera(key, function (d, obj) {
        ...
        var icon = node
          .children(".eleTree-node-content")
          .find(".eleTree-node-content-icon .layui-icon");
        ...
        if (!icon.hasClass("icon-rotate")) {
          var expandOnClickNode = options.expandOnClickNode
            ? node.children(".eleTree-node-content")
            : node
                .children(".eleTree-node-content")
                .children(".eleTree-node-content-icon");
        ...
        node
          ...
          .each(function (index, item) {
            if (
              data[options.request.key] == $(item).data(options.request.key)
            ) {
              isExist = true;
            }
          });
    ...
    insertBefore: function (key, data) {
      ...
      this.keySearchToOpera(key, function (d, obj) {
        ...
        obj.len++;
        var eletreeStatus =
          node.parent(".eleTree-node-group").length === 0
            ? "0"
            : node
                .parent(".eleTree-node-group")
                .parent(".eleTree-node")
    ...
    getChecked: function (leafOnly, includeHalfChecked) {
      ...
      if (leafOnly) {
        el = options.elem
          .find(".layui-icon.leaf-icon")
          .parent(".eleTree-node-content-icon")
          .siblings("input.eleTree-hideen[eletree-status='1']");
      } else if (includeHalfChecked) {
        el = options.elem.find(
          "input.eleTree-hideen[eletree-status='1'],input.eleTree-hideen[eletree-status='2']"
        );
      } else {
    ...
    unCheckArrNodes: function (arr) {
      ...
      var dataChecked = [];
      options.elem
        .find(".eleTree-hideen[eletree-status='1']")
        .each(function (index, item) {
          var id = $(item)
            .parent(".eleTree-node-content")
            .parent(".eleTree-node")
    ...
    expandNode: function (key) {
      var options = this.config;
      var parentsEl = options.elem.find(
        "[data-" + options.customKey + "='" + key + "']"
      );
      var isExpand = parentsEl
        .children(".eleTree-node-content")
        .find(".eleTree-node-content-icon>.layui-icon.layui-icon-triangle-r")
        .hasClass("icon-rotate");
      ...
      if (this.isRenderAllDom) {
        ...
        parentsEl
          .find(".layui-icon.layui-icon-triangle-r")
          .addClass("icon-rotate");
        return false;
      }
    ...
    nodeEvent: function () {
      ...
      // 节点被拖拽的回调事件
      options.draggable &&
        options.elem.on("mousedown", ".eleTree-node-content", function (e) {
          var time = 0;
          var eleNode = $(this).parent(".eleTree-node");
          ...
          $(document)
            .on("mousemove.docMove", function (e) {
              // t为了区别click事件
              time++;
              if (time > 2) {
            ...
            .on("mouseup.docUp", function (e) {
              ...
              // 当前点击的是否时最外层
              var isCurrentOuterMost = eleNode
                .parent()
                .get(0)
                .isEqualNode(options.elem.get(0));
              // 目标是否时最外层
              var isTargetOuterMost = $(e.target)
                .get(0)
                .isEqualNode(options.elem.get(0));
              ...
              // 判断是否超出边界
              if (
                target.parents(options.elem).length === 0 &&
                !isTargetOuterMost
              ) {
                return;
              }
              ...
              temNode
                .children(".eleTree-node-content")
                .css("padding-left", floor * options.indent + "px");
              // 计算线条的left
              if (options.showLine) {
                // 判断目标是否是最外层，是的话隐藏线条
                if (floor === 0) {
                  temNode
                    .children(
                      ".eleTree-node-verticalline,.eleTree-node-horizontalline"
                    )
                    .hide();
                } else {
                  temNode
                    ...
                    .css("left", options.indent * (floor - 1) + 9 + "px")
                    .show();
                }
              }
              ...
              temNode.find(".eleTree-node").each(function (index, item) {
                ...
                $(item)
                  .children(".eleTree-node-content")
                  .css("padding-left", f * options.indent + "px");
                options.showLine &&
                  $(item)
                    .children(
                      ".eleTree-node-verticalline,.eleTree-node-horizontalline"
                    )
                    .css("left", options.indent * (f - 1) + 9 + "px")
                    .show();
              });
              // 原dom去三角
              ...
              // 当前的增加三角
              var cLeaf =
                target.children(".eleTree-node-group").children(".eleTree-node")
                  .length === 1;
              cLeaf &&
                target
                  .children(".eleTree-node-content")
                  ...
                  .removeAttr("style");
              // 判断当前是否需要显示
              var isShowNode = target
                .children(".eleTree-node-content")
                .find(".layui-icon")
                .hasClass("icon-rotate");
              (isTargetOuterMost || isShowNode) &&
                target.children(".eleTree-node-group").show();
    ...
    rightClickMenu: function () {
      ...
      var menuStr = [
        ...
        $.inArray("copy", options.contextmenuList) !== -1
          ? '<li class="copy"><a href="javascript:;">复制</a></li>'
          : "",
        $.inArray("add", options.contextmenuList) !== -1 ||
        $.inArray("add.async", options.contextmenuList) !== -1
          ...
          : "",
        $.inArray("edit", options.contextmenuList) !== -1 ||
        $.inArray("edit.async", options.contextmenuList) !== -1
          ? '<li class="edit"><a href="javascript:;">修改</a></li>'
          : "",
        $.inArray("remove", options.contextmenuList) !== -1 ||
        $.inArray("remove.async", options.contextmenuList) !== -1
      ...
      options.elem
        .off("contextmenu")
        .on("contextmenu", ".eleTree-node-content", function (e) {
          var that = this;
          e.stopPropagation();
          e.preventDefault();
          // 添加active背景
          ...
          $("#tree-menu")
            .find("li.append,li.insertAfter,li.insertBefore")
            .hide();
          $("#tree-menu")
            .find(":not(li.append,li.insertAfter,li.insertBefore)")
            .show();
          $("#tree-menu")
            .css({
          ...
          $("#tree-menu li.add")
            ...
            .on("click", function (e) {
              ...
              $(this)
                .hide()
                .siblings("li:not(.append,.insertAfter,.insertBefore)")
                .hide();
            ...
            $("#tree-menu li." + s)
              ...
              .on("click", function (e) {
                ...
                $.each(nodeArr, function (i, itemNode) {
                  if (
                    obj[options.request.key] ===
                    $(itemNode).data(options.request.key)
                  ) {
                    var label = $(itemNode)
                      .children(".eleTree-node-content")
                      .children(".eleTree-node-content-label")
                    ...
                    var inp =
                      ...
                      obj[options.request.name] +
                      "' class='eleTree-node-content-input' />";
                    label.after(inp);

                    label
                      .siblings(".eleTree-node-content-input")
                      .focus()
                      .select()
                      .off()
                      .on("blur", function () {
                        ...
                        layui.event.call(
                          ...
                          "node" + val + "(" + _self.filter + ")",
                          {
                            ...
                            setData: function (o) {
                              ...
                              d[options.request.name] =
                                newObj[options.request.name];
                              d[options.request.key] =
                                newObj[options.request.key];
                              // 修改dom
                              $(inpThis)
                                .siblings(".eleTree-node-content-label")
                                .text(newObj[options.request.name])
                                .show();
                              $(itemNode).attr(
                                "data-" + options.customKey,
                                newObj[options.request.key]
                              ); // 改变页面上面的显示的key，之后可以获取dom
                              $(itemNode).data(
                                options.request.key,
                                newObj[options.request.key]
                              ); // 改变data数据，之后可以通过data获取key
                              $(inpThis).remove();
                        ...
                        if (
                          ...
                          if (isStop) return;
                          // 修改数据
                          _self.reInitData($(itemNode)).currentData[
                            options.request.name
                          ] = v;
                          // 修改dom
                          $(this)
                  ...
                  layui.event.call(
                    ...
                    "nodeEdit(" + _self.filter + ")",
                    {
                      ...
                      async: function () {
                        if (isStop) return;
                        // 修改数据
                        _self.reInitData(eleNode).currentData[
                          options.request.name
                        ] = val;
                        // 修改dom
                        $(inpThis)
    ...
    search: function (value) {
      var options = this.config;
      if (
        !options.searchNodeMethod ||
        typeof options.searchNodeMethod !== "function"
      ) {
        return;
      }
        ...
        data.forEach(function (val, index) {
          ...
          if (!val.visible) {
            ...
            if (
              ...
              childSomeShow = val[options.request.children].some(function (
                v,
                i
              ) {
                return v.visible;
          ...
          var el = options.elem.find(
            "[data-" +
              options.customKey +
              "='" +
              val[options.request.key] +
              "']"
          );
          if (val.visible) {
            ...
            var parentEl = el
              .parent(".eleTree-node-group")
              .parent(".eleTree-node");
            var isParentOpen = parentEl
              .children(".eleTree-node-content")
              .children(".eleTree-node-content-icon")
              .children(".layui-icon.layui-icon-triangle-r")
              .hasClass("icon-rotate");
            if (
              (parentEl.length > 0 && isParentOpen) ||
              parentEl.length === 0
            ) {
              el.show();
            }
