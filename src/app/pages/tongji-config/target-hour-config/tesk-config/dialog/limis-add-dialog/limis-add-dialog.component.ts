...
export class LimisAddDialogComponent implements OnInit,AfterViewInit {
 ...
 // table配置及数据
      ...
      ol: {
        ...
        onComponentInitFunction:(instance)=>{
          instance.del.subscribe(f=>{
            ...
            this.dialogService.open(this.dialogTemp,{context:`是否删除${f.row.no}?`}).onClose.subscribe(g=>{
              if(g == 1){
                this.http.callRPC('lims_delete_sy_task','lims_delete_sy_task',{
  ...
  constructor(private dialogRef: NbDialogRef<LimisAddDialogComponent>,
    private http:HttpserviceService,private user: UserInfoService,
   ...
   * @param pagejson 
   */
  get_dialog_data(s?:string,pagejson:any={
      limit: 15,
      offset: 0,
      searchstr:'',},
    ){
    ...
    this.http.callRPC('lims_get_wt_order','lims_get_wt_order',params).subscribe((f:any)=>{
        console.log(f)
        ...
        this.rowData = f.result.message[0].message.map(m=>({
          ...
          application_time_format:dateformat(new Date(m.applicanton),'yyyy年MM月dd日'),
          ...
          startTimeFormat:dateformat(new Date(m.taskstarton),'yyyy年MM月dd日'),
        ...
        this.tableDatas.totalPageNumbers = f.result.message[0].numbers[0].numbers;
        if(s == 'init'){
  ...
  get_drawer_data(wtorderno:string){
    ...
    this.http.callRPC('lims_get_wt_order_info','lims_get_wt_order_info',{
      wtorderno:wtorderno
    }).subscribe((f:any)=>{
      console.log('抽屉内数据---->',f);
      if(!f || f.result.message[0].code ==0)return;
  ...
  get_drawer_table_data(){
    return new Observable(s =>{
      let id:any = 0;
      this.http.callRPC('lims_get_sy_task','lims_get_sy_task',{"tasknum":this.current.wtorderno}).subscribe((f:any)=>{
        console.log(f)
        let res = f.result.message[0].message;
        let items = [];
        if(res){
...
// ypname: "yj555"
            let item = {
              ...
              l_no:el.ypid?el.ypid.split(',').map(m =>({key:m,value:''})):[],// [{key:'',value:''}]
              l_name:el.ypname?el.ypname.split(',').map(m =>({key:'',value:m})):[],
              e_context:{devicename:el.devicename||'',deviceid:el.deviceid||''},
  ...
  insert_drawer_table_data(){
    ...
    let item = {
      ...
      executor:Array.isArray(res.executor)?res.executor.join(','):res.executor,
...
// group :''//
    this.http.callRPC('lims_insert_sy_task','lims_insert_sy_task',item).subscribe((f:any)=>{
        console.log(f)
        this.get_drawer_table_data().subscribe(()=>{
          this.loading_dialog = false;
        })
  ...
  get_equipment_data(){
    ...
    this.http.callRPC('get_lims_groups','get_lims_groups',{groups: this.current.groups}).subscribe((f:any)=>{
      // console.log(f)
      let res = f.result.message[0].message;
  ...
  add(){
    if(!this.current.entry){
      // 试验条目名称
      this.dialogService.open(this.dialogTemp,{context:`试验条目名称为必录项!`}).onClose.subscribe(g=>{
      })
      return;
    }
    this.loading_dialog = true;
    this.get_drawer_table_data().subscribe(id=>{
      console.log(id)
      this.current.no = this.current.wtorderno + '-' + id;
