...
export class RoleComponent implements OnInit {
    ...
    switch (rowdata.length) {
      case 1:
        ...
        this.dialogService
          .open(AddRoleComponent, {
            closeOnBackdropClick: false,
            context: { rowdata: JSON.stringify(rowData) },
          })
          .onClose.subscribe((name) => {
  ...
  query(inpuvalue?) {
    ...
    if (role_name != "") {
      ...
      this.loading = true;
      this.http
        .callRPC("role", "sys_search_role", columns)
        .subscribe((result) => {
          var res = result["result"]["message"][0];
          this.loading = false;
          if (res["code"] === 1) {
            ...
            this.RecordOperation(
              "搜索",
              0,
              "角色名称(role_name):" + String(data_info)
            );
          }
        });
    } else {
        ...
        this.http.callRPC(table, method, colums).subscribe((result) => {
          ...
          if (baseData["code"] === 1) {
            localStorage.setItem(
              SYSROLEMENU,
              JSON.stringify(baseData["message"])
            );
            // 得到特定的树状结构数据
            this.sysrolemenu_to_tree_v2(baseData["message"]).subscribe(
              (treedata) => {
    ...
    layui.use(["layer", "form", "eleTree"], function () {
      ...
      eleTree.on("nodeChecked(treedata)", function (d) {
        ...
        if (d.isChecked) {
          ...
          if (
            d.data["currentData"] != undefined &&
            d.data["currentData"]["children"] != null
          ) {
            // 展开所有
  ...
  inttable(event?) {
    ...
    this.http.callRPC(table, method, columns).subscribe((result) => {
      ...
      if (baseData["code"] === 1) {
        ...
        this.tableDatas.rowData = this.gridData;
        var totalpagenumbers = baseData["numbers"]
          ? baseData["numbers"][0]["numbers"]
          : "未得到总条数";
        this.tableDatas.totalPageNumbers = totalpagenumbers;
