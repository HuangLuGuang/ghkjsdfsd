...
export class NewUserEmployeeComponent implements OnInit {
  ...
  constructor(
    ...
    this.http
      .callRPC("", "sys_get_all_role_and_group", {})
      .subscribe((result) => {
  ...
  change_password(loginname) {
    this.dialogService
      .open(ChangePassowrdForallComponent, {
  ...
  add() {
    this.dialogService
      .open(UserEmployeeComponent, {
        closeOnBackdropClick: false,
        context: {
          rowdata: JSON.stringify("add"),
          res: JSON.stringify(""),
          goups: JSON.stringify(""),
        },
      })
      .onClose.subscribe((istrue) => {
        ...
        .onClose.subscribe((istrue) => {
          if (istrue) {
            ...
            try {
              ...
              this.http
                .callRPC("employee", "sys_delete_employees", rowdata)
                .subscribe((result) => {
      ...
      case 1:
        this.dialogService
          .open(UserEmployeeComponent, {
            closeOnBackdropClick: false,
            autoFocus: true,
            context: { rowdata: JSON.stringify(rowdata[0]) },
          })
          .onClose.subscribe((istrue) => {
            if (istrue) {
              this.gridData = [];
