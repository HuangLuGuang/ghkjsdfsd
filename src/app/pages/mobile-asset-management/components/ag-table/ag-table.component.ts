...
export class AgTableComponent implements OnInit {
  ...
  @Output() private clickrow = new EventEmitter<any>(); // 点击的行
  @Output() private selectedrow = new EventEmitter<any[]>(); // 选中的行，可多选！
  ...
  onSelectionChanged(event) {
    ...
    this.selectedrow.emit(this.selectedRows);
  }
      ...
      data.forEach((element) => {
        ...
        if (keys != []) {
          for (let k of keys) {
            // 去掉数值为null、undefind的
            var element_value =
              element[k] == "null" || element[k] == "undefind"
                ? ""
                : element[k];
