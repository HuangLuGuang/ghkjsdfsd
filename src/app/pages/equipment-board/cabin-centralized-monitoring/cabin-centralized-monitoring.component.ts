...
export class CabinCentralizedMonitoringComponent implements OnInit {
  ...
  resize = () => {
    setTimeout(() => {
      ...
      if (document.getElementById("cabin_discharge_chart_2"))
        echarts
          .init(document.getElementById("cabin_discharge_chart_2"))
          .resize();
      if (document.getElementById("cabin_discharge_chart_1"))
        echarts
          .init(document.getElementById("cabin_discharge_chart_1"))
          .resize();
  ...
  get_environmental_warehouse_jinhua() {
    ...
    this.subscribeList.jinhua = this.http
      .callRPC(
        ...
        { device: this.deviceid_jinhua, arr: param_jinhua.join(",") }
      )
      .subscribe((g: any) => {
        ...
        // 冷却水压力
        this.jinhua_en[0].dataLine.value =
          data.mwsmart_main_soaking_s_vw206 || 0;
        //冷却水温度
        this.jinhua_en[1].dataLine.value =
          data.mwsmart_main_soaking_s_vw208 || 0;
        // //温度实际值
        ...
        this.gauge[3].dataLine.color[0] = [
          (data.mwsmart_main_soaking_s_vw202 || 0) / this.gauge[3].dataLine.max,
          "#203add",
        ];
  ...
  get_jinhua_list() {
    ...
    let res,
      arr = this.jinhua_en_charts;
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.deviceid_jinhua,
        arr: "mwsmart_main_soaking_s_vw208,mwsmart_main_soaking_s_vw206,mwsmart_main_soaking_s_vw200",
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        arr[0].value = res[0].mwsmart_main_soaking_s_vw208.map(
          (m) => m[0] || 0
        ); //冷却水温度
        arr[1].value = res[1].mwsmart_main_soaking_s_vw206.map(
          (m) => m[0] || 0
        ); //冷却水压力
        arr[2].value = res[2].mwsmart_main_soaking_s_vw200.map(
          (m) => m[0] || 0
        ); //温度
        let max_index = 0,
          max = [];
        ...
        this.jinhua_en_xData = xarr.map((m) =>
  ...
  get_ATEC() {
    let res,
      data: any = {};
    this.subscribeList.atec = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.deviceid_ATEC, arr: param_ATEC.join(",") }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        this.gauge[0].dataLine.color[0] = [
          (data.realtime_temp || 0) / this.gauge[0].dataLine.max,
          "#203add",
        ];
        //湿度
        this.gauge[1].dataLine.value = data.realtime_humidity || 0;
        this.gauge[1].dataLine.color[0] = [
          (data.realtime_humidity || 0) / this.gauge[1].dataLine.max,
          "#203add",
        ];
        // 微压差
        this.gauge[2].dataLine.value = data.micro_pressure_pv || 0;
        this.gauge[2].dataLine.color[0] = [
          (data.micro_pressure_sp || 0) / this.gauge[2].dataLine.max,
          "#203add",
        ];
        [0, 1, 2].forEach((f) => {
  ...
  get_ATEC_list() {
    ...
    this.http
      ...
      .subscribe((g: any) => {
        ...
        this.atec_chart[1].value = res[1].realtime_humidity.map(
          (m) => m[0] || 0
        );
        ...
        if (document.getElementById("cabin_discharge_chart_1")) {
          let myChart_9 = echarts.init(
            document.getElementById("cabin_discharge_chart_1")
          );
          equipment_four_road.create_broken_line(
            {
              title: "温湿度曲线",
  ...
  get_TempHumidity(device, arr) {
    let res;
    this.subscribeList[device] = this.http
      .callRPC("get_temperature", library + "get_temperature", {
        device: device,
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message[0]
          ? g.result.message[0].message[0]
          : {};
