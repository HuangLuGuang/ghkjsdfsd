...
export class EquipmentAvlAtecComponent implements OnInit {
  ...
  get_avl_igem() {
    let res,
      data: any = {};
    this.subscribeList.igem = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.light_deviceid, arr: avl_igem_param.join(",") }
      )
      .subscribe((g: any) => {
  ...
  get_atec() {
    let res,
      data: any = {};
    this.subscribeList.light = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.aetc_deviceid, arr: light_param.join(",") }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        this.gauge[4].dataLine.color[0] = [
          (data.temp_setpoint || 0) / this.gauge[4].dataLine.max,
          "#203add",
        ];
        this.gauge[5].dataLine.value = data.realtime_humidity || 0;
        this.gauge[5].dataLine.color[0] = [
          (data.humidity_setpoint || 0) / this.gauge[5].dataLine.max,
          "#203add",
        ];
        this.gauge[6].dataLine.value = data.micro_pressure_pv || 0;
        this.gauge[6].dataLine.color[0] = [
          (data.micro_pressure_sp || 0) / this.gauge[6].dataLine.max,
          "#203add",
        ];
  ...
  get_atec_list() {
    let res,
      xdata = [],
      data = this.light_chart;
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.aetc_deviceid,
        arr: "realtime_temp,realtime_humidity,micro_pressure_pv",
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        if (data[0].value.length > data[1].value.length) {
          ...
          xdata = res[2].micro_pressure_pv.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        }
        ...
        if (document.getElementById("avl_discharge_chart_1")) {
          let myChart_9 = echarts.init(
            document.getElementById("avl_discharge_chart_1")
          );
  ...
  get_avl_d() {
    let res,
      data: any = {};
    this.subscribeList.avl_d = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.avl_deviceid, arr: avl_param.join(",") }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        [this.gauge[0], this.gauge[1], this.gauge[2], this.gauge[3]].forEach(
          (f) => {
