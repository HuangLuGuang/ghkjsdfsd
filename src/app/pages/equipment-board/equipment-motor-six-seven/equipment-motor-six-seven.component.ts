...
export class EquipmentMotorSixSevenComponent implements OnInit {
  ...
  get_motor_param() {
    ...
    let res,
      data: any = {},
      chart;
    this.subscribeList.motor = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.boyang_deviceid, arr: boyang_param.join(",") }
      )
      .subscribe((g: any) => {
  ...
  get_motor_list() {
    ...
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.boyang_deviceid,
        arr: "urms4,irms4,prms4",
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        if (
          this.threePhase_attrs[0].value.length >
          this.threePhase_attrs[1].value.length
        ) {
          this.threePhase_xData = res[0].urms4.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        } else if (
          this.threePhase_attrs[1].value.length >
          this.threePhase_attrs[2].value.length
        ) {
          this.threePhase_xData = res[1].irms4.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        } else {
          this.threePhase_xData = res[2].prms4.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        }
        ...
        if (chart)
          equipment_four_road.create_real_discharge(
            {
              attrs: this.threePhase_attrs,
              xData: this.threePhase_xData,
              title: "直流电压/电流/功率",
            },
            echarts.init(chart)
          );
  ...
  get_left_data() {
    ...
    this.subscribeList.left = this.http
      ...
      .subscribe((g: any) => {
        ...
        this.HealthParam_left_chart[0].value =
          res[0].liquidsupplytemperaturetop.map((m) => m[0] || 0);
        this.HealthParam_left_chart[1].value = res[1].liquidsupplyflowtop.map(
          (m) => m[0] || 0
        );
        this.HealthParam_left_chart[2].value =
          res[2].liquidsupplytemperaturedown.map((m) => m[0] || 0);
        this.HealthParam_left_chart[3].value = res[3].liquidsupplyflowdown.map(
          (m) => m[0] || 0
        );
        ...
        this.HealthParam_left_xdata = max[0].map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );
        ...
        if (document.getElementById("motor_chart_1"))
          equipment_four_road.create_real_discharge(
            {
              attrs: this.HealthParam_left_chart,
              xData: this.HealthParam_left_xdata,
              title: "冷却水温度/流速",
            },
            echarts.init(document.getElementById("motor_chart_1"))
          );
        ...
        this.HealthParam_left.forEach((f: any, i) => {
          ...
          //判断获取到的数据中心长度最长的
          f.dataLine.value =
            length > 0 ? this.HealthParam_left_chart[i].value[length - 1] : 0;
          equipment_four_road.create_temp_h_1_p_gauge(
            f.dataLine,
            echarts.init(document.getElementById(f.id))
          );
    ...
    this.subscribeList.get_line_speed_torque = this.http
      ...
      .subscribe((f: any) => {
        ...
        setTimeout(() => {
          ...
          this.speedTorque_xData = res[i][c].map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
          if (chart)
            equipment_four_road.create_motor_chart(
              {
                xData: this.speedTorque_xData,
                data: this.speedTorque_attrs,
                title: "转速/扭矩曲线",
              },
              echarts.init(chart)
            );
