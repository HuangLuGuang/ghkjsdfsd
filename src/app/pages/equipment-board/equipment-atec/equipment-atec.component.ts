...
export class EquipmentAtecComponent implements OnInit {
  ...
  get_atec(){
    ...
    this.http.callRPC('get_device_mts_realtimedata',library+'get_device_mts_realtimedata',{
      device:this.device,arr:atec.join(',')
    }).subscribe((g:any)=>{
  ...
  get_atec_list(){
    ...
    this.http.callRPC('device_realtime_list',library+'device_realtime_list',{
      deviceid:this.device,arr:atec_list.join(',')
    }).subscribe((g:any)=>{
      if(g.result.error || g.result.message[0].code == 0)return;
      res = g.result.message[0].message;
      this.atec.attrs[0].value = res[0]['realtime_temp'].map(m =>(m[0]||0));
      this.atec.attrs[1].value = res[1]['realtime_humidity'].map(m =>(m[0]||0));
