...
export class EquipmentGlassLiftComponent implements OnInit {
  ...
  get_atec_temp() {
    ...
    this.http
      .callRPC(
        ...
        { device: this.deviceid_cabin, arr: atec.join(",") }
      )
      .subscribe((g: any) => {
  ...
  get_atec_temp_list() {
    let chart,
      arr = this.atec;
    this.subscribeList.get_line_speed_torque = this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.deviceid_cabin,
        arr: "realtime_temp,realtime_humidity",
      })
      .subscribe((f: any) => {
  ...
  get_sky() {
    let res;
    this.http
      .callRPC("get_sky_rate", library + "get_sky_rate", {
        deviceid: this.deviceid_sky,
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        ...
        if (document.getElementById("progress_1"))
          equipment_four_road.progress(
            { plan: 100, now: this.sky_rate.one?.rate || 0 },
            echarts.init(document.getElementById("progress_1"))
          );
        if (document.getElementById("progress_2"))
          equipment_four_road.progress(
            { plan: 100, now: this.sky_rate.two?.rate || 0 },
            echarts.init(document.getElementById("progress_2"))
          );
        if (document.getElementById("progress_3"))
          equipment_four_road.progress(
            { plan: 100, now: this.sky_rate.three?.rate || 0 },
            echarts.init(document.getElementById("progress_3"))
          );
        if (document.getElementById("progress_4"))
          equipment_four_road.progress(
            { plan: 100, now: this.sky_rate.four?.rate || 0 },
            echarts.init(document.getElementById("progress_4"))
          );
      });
  }
  ...
  get_sky_list() {
    this.http
      ...
      .subscribe((f: any) => {
        ...
        xdata = res[index][key].map((m) =>
          dateformat(new Date(m[1]), "hh:mm:ss")
        );
        setTimeout(() => {
          this.current.attrs[0].value = res[0].station1_rise_current.map(
            (m) => m[0] || 0
          );
          this.current.attrs[1].value = res[1].station2_rise_current.map(
            (m) => m[0] || 0
          );
          this.current.attrs[2].value = res[2].station3_rise_current.map(
            (m) => m[0] || 0
          );
          this.current.attrs[3].value = res[3].station4_rise_current.map(
            (m) => m[0] || 0
          );
          if (document.getElementById("sky_line_1")) {
            equipment_four_road.create_real_discharge(
              { attrs: this.current.attrs, xData: xdata },
              echarts.init(document.getElementById("sky_line_1"))
            );
          }
        }, 20);

        this.voltage.attrs[0].value = res[4].station1_lifter_voltage.map(
          (m) => m[0] || 0
        );
        this.voltage.attrs[1].value = res[5].station2_lifter_voltage.map(
          (m) => m[0] || 0
        );
        this.voltage.attrs[2].value = res[6].station3_lifter_voltage.map(
          (m) => m[0] || 0
        );
        this.voltage.attrs[3].value = res[7].station4_lifter_voltage.map(
          (m) => m[0] || 0
        );

        if (document.getElementById("sky_line_2")) {
          equipment_four_road.create_real_discharge(
            { attrs: this.voltage.attrs, xData: xdata },
            echarts.init(document.getElementById("sky_line_2"))
          );
        }
      });
  }
