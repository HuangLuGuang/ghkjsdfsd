...
export class EquipmentMahaComponent implements OnInit {
  ...
  resize=()=>{
    setTimeout(() => {
      ['cang_pie_5','cang_pie_6','cang_chart_1','dashboard_67','line_chart_12_67'].forEach(f=>{
  ...
  get_gas(){
    ...
    this.subscribeList.get_line_coolingWater = this.http.callRPC('device_realtime_list',library+'device_realtime_list',
    {
      ...
      setTimeout(() => {
        this.gas_cang.ch = arr[0].value.length > 0 ?arr[0].value[arr.length -1]:0;
        this.gas_cang.co = arr[1].value.length > 0 ?arr[1].value[arr.length -1]:0;

        if(document.getElementById('cang_pie_5'))
          equipment_four_road.create_motor_temperature( {value:arr[0].value.length > 0 ?arr[0].value[arr.length -1]:0,title:'一氧化碳',unit:'%'},
          echarts.init(document.getElementById('cang_pie_5')));
        if(document.getElementById('cang_pie_6'))
          equipment_four_road.create_motor_temperature( {value:arr[1].value.length > 0 ?arr[1].value[arr.length -1]:0,title:'碳氢',unit:'%'},
      ...
      xAxisData = res[index][key].map(m => (dateformat(new Date(rTime(m[1])),'hh:mm:ss')))

      if(document.getElementById('cang_chart_1'))
        equipment_four_road.create_real_discharge({attrs:this.gas_chart,xData:xAxisData},
    ...
    this.subscribeList.get_line_coolingWater = this.http.callRPC('get_device_mts_realtimedata',library+'get_device_mts_realtimedata',
    {
      ...
      if(chart)
        equipment_four_road.create_real_dashboard([{
          name: '功率',unit: 'Kw',value:data.power_total||0
        },{
          name: '速度',unit: 'km/h',value:data.speed_total||0
        },{
          name: '牵引力',unit: 'NW',value:data.tractive_force_total||0
        }],echarts.init(chart));
  ...
  get_maha_list(){
    ...
    this.http.callRPC('device_realtime_list',library+'device_realtime_list',
    {
      ...
      if(attrs[0].data.length > attrs[1].data.length){
        ...
        xdata = res[1].tractive_force_total.map(m => (dateformat(new Date(rTime(m[1])),'hh:mm:ss')));
      }
      
      chart = document.getElementById('line_chart_12_67');
     if(chart)
      equipment_four_road.create_motor_chart({
        xData:xdata,data:attrs,title:'速度/牵引力曲线'},
        echarts.init(chart));
    });
  }
