...
export class EquipmentMotorSystemComponent implements OnInit {
  ...
  get_experimentParams() {
    ...
    let res;
    this.subscribeList.exp = this.http
      .callRPC("get_avl_temperature", library + "get_avl_temperature", {
...
torque: 0.151 扭矩
  ...
  get_right() {
    ...
    let res, chart;
    this.subscribeList.right = this.http
      .callRPC("get_avl_parameter", library + "get_avl_parameter", {
        deviceid: this.deviceid,
      })
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
  ...
  get_line_busbar() {
    ...
    let chart;
    this.subscribeList.get_line_busbar = this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.deviceid,
        arr: "pa_udc1,pa_idc1,pa_p1",
      })
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
        let res = f.result.message[0].message;
        ...
        this.threePhase_xData = res[i][c].map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );
