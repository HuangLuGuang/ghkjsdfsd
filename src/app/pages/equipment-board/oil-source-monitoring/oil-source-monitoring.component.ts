...
export class OilSourceMonitoringComponent implements OnInit {
  ...
  equipIntroduceList = [{ title: "" }, { title: "" }, { title: "" }];
  ...
  get_HPU() {
    ...
    let res;
    this.subscribeList.xhq_hpu = this.http
      .callRPC("get_hpu", library + "get_hpu", { deviceid: "" })
      .subscribe((f: any) => {
        ...
        res.forEach((el) => {
          ...
          let i = parseInt(
            el[0].deviceid.split("_")[el[0].deviceid.split("_").length - 1]
          );
        ...
        let HPUlist_series = this.HPUlist.find(
          (f) => this.HPUselect.value == f.name
        );
        if (document.getElementById("pumpClick"))
          oilsrouce.create_gauge_3(
            HPUlist_series.series,
            echarts.init(document.getElementById("pumpClick")),
            "big"
          );
        this.HPUlist.forEach((f) => {
  ...
  get_Error_Message() {
    ...
    this.subscribeList.error_message = this.http
      .callRPC("get_hpu_warninglog", library + "get_hpu_warninglog", {})
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
        let color = "white";
        let s = "";
        this.equip_alarm = f.result.message[0].message
          .sort((h, c) => c - h)
          .map((g) => {
            ...
            return {
              data: [g.source.split(" ")[0], g.message, time],
              s: "",
              color: color,
            };
          });
  ...
  get_cleanlinss() {
    ...
    let i = this.HPUselect.value.match(/\d{1,}/g),
      res;
    this.subscribeList.cleanlinss = this.http
      .callRPC("get_particle", library + "get_particle", {
        deviceid: "device_hpu_0" + i[0],
      })
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
  ...
  get_cleanlinss_list() {
    ...
    this.subscribeList.get_line_speed_torque = this.http
      ...
      .subscribe((f: any) => {
        ...
        this.xdata_cleanliss = xarr.map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );

        if (chart) {
          equipment_four_road.create_real_discharge(
            { attrs: this.attrs_cleanliss, xData: this.xdata_cleanliss },
            echarts.init(chart)
          );
        }
      });
  ...
  get_Water() {
    ...
    let i = this.HPUselect.value.match(/\d{1,}/g),
      res;
    this.subscribeList.water = this.http
      .callRPC("get_water", library + "get_water", {
        deviceid: "device_hpu_0" + i[0],
      })
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
  ...
  get_water_list() {
    ...
    let i = this.HPUselect.value.match(/\d{1,}/g),
      res,
      arr = this.HE_Water.attrs;
    this.subscribeList.get_line_speed_torque = this.http
      .callRPC(
        "device_realtime_list_second",
        library + "device_realtime_list_second",
        { deviceid: "device_hpu_0" + i[0], arr: j.join(",") }
      )
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
        res = f.result.message[0].message;
        ...
        let xarr: any = Object.values(res[max_index])[0];
        this.HE_Water.xData = xarr.map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );

        if (document.getElementById("discharge_chart_2")) {
          let myChart_9 = echarts.init(
            document.getElementById("discharge_chart_2")
          );
          equipment_four_road.create_real_discharge(
            { attrs: this.HE_Water.attrs, xData: this.HE_Water.xData },
            myChart_9
          );
        }
      });
  ...
  get_Radar() {
    ...
    this.subscribeList.rader = this.http
      .callRPC("get_accumulator", library + "get_accumulator", {
        device: "device_accumulator_01",
        arr: arr.join(","),
      })
      .subscribe((f: any) => {
        if (f.result.error || f.result.message[0].code == 0) return;
        ...
        if (radar_1)
          oilsrouce.create_radar(this.radar_1, echarts.init(radar_1));
        let radar_2 = document.getElementById("radar_2");
        if (radar_2)
          oilsrouce.create_radar(this.radar_2, echarts.init(radar_2));
      });
  }
