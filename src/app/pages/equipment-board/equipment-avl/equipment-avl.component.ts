...
export class EquipmentAvlComponent implements OnInit {
  ...
  environmental_real_data = {
    arr: [
      { name: "舱状态", nameEn: "CabinStatus", value: 1 },
      { name: "新风系统", nameEn: "NewSealSystem", value: 1 },
      { name: "转轮排湿", nameEn: "RunnerDehumidi", value: 1 },
      { name: "制冷除湿", nameEn: "RefrigeraDehumidifica", value: 1 },
      { name: "制冷机组1", nameEn: "RefrigerationUnit1", value: 1 },
      { name: "制冷机组2", nameEn: "RefrigerationUnit2", value: 1 },
    ],
  };
  outRenturnWind = [
  ...
  resize = () => {
    setTimeout(() => {
      ...
      if (document.getElementById("temp_humidity_pressure"))
        echarts
          .init(document.getElementById("temp_humidity_pressure"))
          .resize();
  ...
  get_avl_discharge() {
    let res,
      data: any = {};
    this.subscribeList.discharge = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.deviceid_discharge, arr: discharge_param.join(",") }
      )
      .subscribe((g: any) => {
  ...
  get_avl_discharge_list() {
    ...
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.deviceid_discharge,
        arr: discharge_param.join(","),
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        this.discharge_xdata = xdata.map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );
        //只有一个公共组件
        if (document.getElementById("discharge_chart")) {
          let myChart_8 = echarts.init(
            document.getElementById("discharge_chart")
          );
          equipment_four_road.create_real_discharge(
            { attrs: this.discharge_charts, xData: this.discharge_xdata },
            myChart_8
          );
        }
      });
  ...
  get_avl_environmental_warehouse() {
    let res,
      data: any = {};
    this.subscribeList.warehouse = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        {
          device: this.deviceid_environmental,
          arr: environmental_param.join(","),
        }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        ...
        this.outRenturnWind.forEach((f: any, i: number) => {
          if (!echarts.init(document.getElementById("electric_chart_" + i)))
            return;
          let c = echarts.init(document.getElementById("electric_chart_" + i));
          equipment_four_road.create_real_electric(
            {
              text: f.text,
              title: this.language ? f.titleEn : f.title,
              unit: "℃",
            },
            c
          );
  ...
  get_avl_env_list() {
    let res,
      xdata = [],
      data = this.environmental_chart;
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.deviceid_environmental,
        arr: "ct_temp,ct_hum,ct_pressure_set",
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        if (data[0].value.length > data[1].value.length) {
          ...
          xdata = res[2].ct_pressure_set.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        }
        ...
        if (document.getElementById("discharge_chart_1")) {
          let myChart_9 = echarts.init(
            document.getElementById("discharge_chart_1")
          );
          equipment_four_road.create_real_discharge(
            { attrs: data, xData: this.environmental_xdata },
            myChart_9
          );
        }
      });
  ...
  get_avl_speed() {
    let res,
      data: any = {};
    this.subscribeList.speed = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.deviceid_avl_speed, arr: avl_speed_param.join(",") }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
  ...
  get_avl_speed_list() {
    let res,
      xdata: any = [],
      arr = this.avl_speed_chart;
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.deviceid_avl_speed,
        arr: "f,v,p",
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        ...
        this.avl_speed_xdata = xdata.map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );
