...
export class EquipmentShengweiComponent implements OnInit {
  ...
  resize = () => {
    setTimeout(() => {
      ["cabin_pie_4", "cabin_pie_5", "line_chart_1", "cabin_line_4"].forEach(
        (f) => {
  ...
  fourMthree() {
    ...
    this.subscribeList._4400 = this.http
      .callRPC(
        ...
        { device: this.deviceid_4m3, arr: FmT.join(",") }
      )
      .subscribe((g: any) => {
        ...
        if (document.getElementById("cabin_pie_4"))
          equipment_four_road.create_motor_temperature(
            { value: data.temperature_actual || 0, title: "温度", unit: "℃" },
            echarts.init(document.getElementById("cabin_pie_4"))
          );

        if (document.getElementById("cabin_pie_5"))
          equipment_four_road.create_motor_temperature(
            { value: data.humidity_actual || 0, title: "湿度", unit: "RH" },
  ...
  fourMthreeList() {
    this.subscribeList.get_line_coolingWater = this.http
      ...
      .subscribe((f: any) => {
        ...
        let xdata = [];
        this.data_4m3_chart.attrs[0].value = res[0].temperature_actual.map(
          (m) => m[0] || 0
        );
        this.data_4m3_chart.attrs[1].value = res[1].humidity_actual.map(
          (m) => m[0] || 0
        );
        if (
          this.data_4m3_chart.attrs[0].value.length >
          this.data_4m3_chart.attrs[1].value.length
        ) {
          xdata = res[0].temperature_actual.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        } else {
          xdata = res[1].humidity_actual.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        }
        ...
        if (document.getElementById("cabin_line_4"))
          equipment_four_road.create_real_discharge(
            {
              attrs: this.data_4m3_chart.attrs,
              xData: this.data_4m3_chart.xdata,
            },
  ...
  voc() {
    ...
    this.subscribeList._4400 = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.deviceid_voc, arr: voc.join(",") }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
  ...
  vocList() {
    this.subscribeList.get_line_coolingWater = this.http
      ...
      .subscribe((f: any) => {
        ...
        this.data_voc_chart.attrs[0].value = res[0].temperature_actual.map(
          (m) => m[0] || 0
        );
        this.data_voc_chart.attrs[1].value = res[1].testing_temperature_set.map(
          (m) => m[0] || 0
        );
        if (
          this.data_voc_chart.attrs[0].value.length >
          this.data_voc_chart.attrs[1].value.length
        ) {
          xdata = res[0].temperature_actual.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        } else {
          xdata = res[1].testing_temperature_set.map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        }
        ...
        if (document.getElementById("line_chart_1"))
          equipment_four_road.create_real_discharge(
            {
              attrs: this.data_voc_chart.attrs,
              xData: this.data_voc_chart.xdata,
            },
