...
export class EquipmentJinhua4d2cComponent implements OnInit {
  ...
  get_atec() {
    ...
    this.http
      .callRPC(
        ...
        {
          device: this.device_atec,
          arr: Object.values(this.cang_arr).join(","),
        }
      )
      .subscribe((g: any) => {
        ...
        if (document.getElementById(this.atec.humid))
          equipment_four_road.create_motor_temperature(
            { value: this.atec.rhReal, title: "湿度", unit: "%RH" },
  ...
  get_atec_list() {
    ...
    this.http
      .callRPC("device_realtime_list", library + "device_realtime_list", {
        deviceid: this.device_atec,
        arr: Object.values(this.cang_arr_list).join(","),
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        this.atec.attrs[0].value = res[0][this.cang_arr_list.temp].map(
          (m) => m[0] || 0
        );
        this.atec.attrs[1].value = res[1][this.cang_arr_list.rh].map(
          (m) => m[0] || 0
        );

        if (this.atec.attrs[0].value.length > this.atec.attrs[1].value.length) {
          xdata = res[0][this.cang_arr_list.temp].map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        } else {
          xdata = res[1][this.cang_arr_list.rh].map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
        }
  ...
  get_4d2c() {
    ...
    this.http
      ...
      .subscribe((g: any) => {
        ...
        //前盖
        this.list[0].close_status =
          data.mwsmart_1_equipment_lg_close_warning || 0;
        this.list[0].open_status =
          data.mwsmart_1_equipment_lg_open_warning || 0;
        this.list[0].plan = data.mwsmart_1_equipment_lg_total_cycle || 0;
        this.list[0].now = data.mwsmart_1_equipment_lg_current_cycle || 0;
        this.list[0].rate = this.list[0].plan
          ? parseFloat((this.list[0].now / this.list[0].plan).toFixed(2))
          : 0;
        // this.list[0].rate = 20;

        // 前门
        this.list[1].close_status =
          data.mwsmart_1_equipment_fd_close_warning || 0;
        this.list[1].open_status =
          data.mwsmart_1_equipment_fd_open_warning || 0;
        this.list[1].plan = data.mwsmart_1_equipment_fd_total_cycle || 0;
        this.list[1].now = data.mwsmart_1_equipment_fd_current_cycle || 0;
        this.list[1].rate = this.list[1].plan
          ? parseFloat((this.list[1].now / this.list[1].plan).toFixed(2))
          : 0;

        // 后盖
        this.list[2].close_status =
          data.mwsmart_1_equipment_h_close_warning || 0;
        ...
        this.list[2].rate = this.list[2].plan
          ? parseFloat((this.list[2].now / this.list[2].plan).toFixed(2))
          : 0;

        // 后门
        this.list[3].close_status =
          data.mwsmart_1_equipment_rd_close_warning || 0;
        this.list[3].open_status =
          data.mwsmart_1_equipment_rd_current_cycle || 0;
        this.list[3].plan = data.mwsmart_1_equipment_rd_total_cycle || 0;
        this.list[3].now = data.mwsmart_1_equipment_rd_current_cycle || 0;
        this.list[3].rate = this.list[3].plan
          ? parseFloat((this.list[3].now / this.list[3].plan).toFixed(2))
          : 0;

        this.list.forEach((f, i) => {
          if (document.getElementById(`progress_${i + 1}`))
            equipment_four_road.progress(
              { plan: 100, now: f.rate },
              echarts.init(document.getElementById(`progress_${i + 1}`))
            );
        });
      });
  }
  ...
  get_Temp_Hum() {
    ...
    let yearPlanData = [],
      ...
      visibityData = [],
      xAxisData = [];
    this.subscribeList.h_t_h = this.http
      .callRPC("get_temperature", library + "get_temperature_numbers", {
        deviceid: t_h_deviceid || this.th_deviceid,
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        ...
        let temp =
          yearPlanData.length > 0 ? yearPlanData[yearPlanData.length - 1] : 0;
        let hum =
          yearOrderData.length > 0
            ? yearOrderData[yearOrderData.length - 1]
            : 0;
        chart = document.getElementById("atec_pie_5");
        if (chart)
          equipment_four_road.create_motor_temperature(
            { value: temp, unit: "℃", title: "温度" },
            echarts.init(chart)
          );
        chart = document.getElementById("atec_pie_6");
        if (chart)
          equipment_four_road.create_motor_temperature(
            { value: hum, unit: "RH", title: "湿度" },
            echarts.init(chart)
          );
