...
export class EquipmentCtsBsrComponent implements OnInit {
  ...
  get_cts() {
    ...
    this.subscribeList.get_line_coolingWater = this.http
      ...
      .subscribe((f: any) => {
        ...
        arr[0].value = res[0].cts_plc_1_pv_db_temperature_deg_c.map(
          (m) => m[0]
        ); //温度
        arr[1].value = res[1].cts_plc_1_pv_humidity_percent_rh.map((m) => m[0]); //湿度度
        arr[2].value =
          res[2].cts_plc_1_pid_setpoints_in_use_x_pid_setpoints_in_use_01.map(
            (m) => m[0]
          ); //湿度度
        arr[3].value =
          res[3].cts_plc_1_pid_setpoints_in_use_x_pid_setpoints_in_use_00.map(
            (m) => m[0]
          ); //湿度度

        setTimeout(() => {
          this.cts_cang.temp_set =
            arr[2].value.length > 0 ? arr[2].value[arr.length - 1] : 0;
          this.cts_cang.hum_set =
            arr[3].value.length > 0 ? arr[3].value[arr.length - 1] : 0;
  ...
  get_bsr_station() {
    ...
    this.subscribeList.real = this.http
      .callRPC("get_device_mts_status", library + "get_device_mts_status", {
        device: this.device_bsr,
        arr: bsr.join(","),
      })
      .subscribe((g: any) => {
        ...
        res.forEach((el) => {
          data.push([
            el.stationname,
            {
              value: el.stationstatus,
              color: el.stationstatus == 1 ? "green" : "#3b3838",
              id: "circle",
            },
            {
              value: el.interlock,
              color: el.interlock == 1 ? "green" : "#3b3838",
              id: "strip",
            },
            {
              value: el.programinterlock,
              color: el.programinterlock == 1 ? "green" : "#3b3838",
              id: "strip",
            },
          ]);
        });
  ...
  get_bsr_station_real_data() {
    ...
    this.subscribeList._4400 = this.http
      .callRPC(
        "get_device_mts_realtimedata",
        library + "get_device_mts_realtimedata",
        { device: this.device_bsr, arr: bsr_chart.join(",") }
      )
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
  ...
  get_bsr_station_list_data() {
    ...
    this.subscribeList.get_line_coolingWater = this.http
      ...
      .subscribe((f: any) => {
        ...
        setTimeout(() => {
          this.bsr_temp.data[0].value = res[0].leftfronttemperaturea.map(
            (m) => m[0] || 0
          );
          this.bsr_temp.data[1].value = res[1].leftreartemperaturea.map(
            (m) => m[0] || 0
          );
          this.bsr_temp.data[2].value = res[2].rightfronttemperaturea.map(
            (m) => m[0] || 0
          );
          this.bsr_temp.data[3].value = res[3].rightreartemperaturea.map(
            (m) => m[0] || 0
          );
          dom = document.getElementById("bsr_chart_1");
          let i = 0,
            c = "leftfronttemperaturea";
          if (
            res[0].leftfronttemperaturea.length <
            res[1].leftreartemperaturea.length
          ) {
            (i = 1), (c = "leftreartemperaturea");
          } else if (
            res[1].leftreartemperaturea.length <
            res[2].rightfronttemperaturea.length
          ) {
            (i = 2), (c = "rightfronttemperaturea");
          } else {
            (i = 3), (c = "rightreartemperaturea");
          }
          this.bsr_temp.xdata = res[i][c].map((m) =>
          ...
          if (dom) {
            equipment_four_road.create_real_discharge(
              {
                attrs: this.bsr_temp.data,
                xData: this.bsr_temp.xdata,
                title: "左前/左后/右前/右后温度",
              },
              echarts.init(dom)
            );
        ...
        setTimeout(() => {
          arr.data[0].value = res[4].rightfrontdisplacement.map(
            (m) => m[0] || 0
          );
          arr.data[1].value = res[5].rightreardisplacement.map(
            (m) => m[0] || 0
          );
          let i = 4,
            c = "rightfrontdisplacement";
          if (
            res[4].rightfrontdisplacement.length <
            res[5].rightreardisplacement.length
          ) {
            (i = 5), (c = "rightreardisplacement");
          }
          arr.xdata_right = res[i][c].map((m) =>
            dateformat(new Date(rTime(m[1])), "hh:mm:ss")
          );
          ...
          if (dom) {
            equipment_four_road.create_real_discharge(
              {
                attrs: arr.data.filter((m, i) => i < 2),
                xData: arr.xdata_right,
                title: "右前/右后位移",
              },
              echarts.init(dom)
            );
        ...
        left =
          arr.data[2].value.length > 0
            ? arr.data[2].value[arr.data[2].value.length - 1]
            : 0;
        right =
          arr.data[3].value.length > 0
            ? arr.data[2].value[arr.data[3].value.length - 1]
            : 0;
        let i = 6,
          c = "leftfrontdisplacement";
        if (
          res[6].leftfrontdisplacement.length <
          res[7].leftreardisplacement.length
        ) {
          (i = 7), (c = "leftreardisplacement");
        }
        arr.xdata_left = res[i][c].map((m) =>
          dateformat(new Date(rTime(m[1])), "hh:mm:ss")
        );
        if (dom) {
          equipment_four_road.create_real_discharge(
            {
              attrs: arr.data.filter((m, i) => i > 1),
              xData: arr.xdata_left,
