...
export class EquipmentMotorLujiaoComponent implements OnInit {
  ...
  get_real_data() {
    ...
    this.subscribeList.real = this.http
      .callRPC(
        ...
        { device: this.deviceid, arr: real.join(",") }
      )
      .subscribe((g: any) => {
        ...
        setTimeout(() => {
          ...
          if (chart) {
            equipment_four_road.create_motor_temperature(
              { value: data.dynowinding1 || 0, unit: "℃" },
              echarts.init(chart)
            );
          }
          ...
          if (chart) {
            equipment_four_road.create_motor_temperature(
              { value: data.dynowinding2 || 0, unit: "℃" },
              echarts.init(chart)
            );
          }
          ...
          if (chart) {
            equipment_four_road.create_motor_temperature(
              { value: data.dynowinding3 || 0, unit: "℃" },
              echarts.init(chart)
            );
          }
        }, 10);
  ...
  get_list() {
    this.subscribeList.list = this.http
      ...
      .subscribe((f: any) => {
        ...
        let res = f.result.message[0].message,
          chart;
        setTimeout(() => {
          this.experiment_attrs[0].value = res[0].dynowinding1.map(
            (m) => m[0] || 0
          );
          this.experiment_attrs[1].value = res[1].dynowinding2.map(
            (m) => m[0] || 0
          );
          this.experiment_attrs[2].value = res[2].dynowinding3.map(
            (m) => m[0] || 0
          );
          let i = 0,
            c = "dynowinding1";
          ...
          this.experiment_xdata = res[i][c].map((m) =>
          ...
          if (chart)
            equipment_four_road.create_real_discharge(
              {
                attrs: this.experiment_attrs,
                xData: this.experiment_xdata,
                title: "线圈温度",
              },
              echarts.init(chart)
            );
        ...
        setTimeout(() => {
          ...
          this.speedTorque_attrs[1].data = res[3].spddyno1.map(
            (m) => m[0] || 0
          );
          let i = 4,
            c = "tqdyno1";
        ...
        this.bearing_attrs[0].value = res[5].bearing_in_de.map(
          (m) => m[0] || 0
        );
        this.bearing_attrs[1].value = res[6].bearing_in_nde.map(
          (m) => m[0] || 0
        );
        this.bearing_attrs[2].value = res[7].bearing_out_de.map(
          (m) => m[0] || 0
        );
        this.bearing_attrs[3].value = res[8].bearing_out_nde.map(
          (m) => m[0] || 0
        );
        let max_index = 0,
          max = [];
        ...
        if (chart) {
          equipment_four_road.create_real_discharge(
            {
              xData: this.bearing_xData,
              attrs: this.bearing_attrs,
              title: "转速/扭矩曲线",
            },
            echarts.init(chart)
          );
  ...
  get_device_mts_timerangedata() {
    ...
    this.subscribeList.h_t_h = this.http
      .callRPC("get_temperature", library + "get_temperature_numbers", {
        deviceid: t_h_deviceid || this.t_h_deviceid,
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        ...
        if (chart)
          equipment_four_road.create_motor_temperature(
            { value: this.temp, unit: "℃", title: "温度" },
            echarts.init(chart)
          );
        chart = document.getElementById("humidity");
        if (chart)
          equipment_four_road.create_motor_temperature(
            { value: this.hum, unit: "RH", title: "湿度" },
            echarts.init(chart)
          );
