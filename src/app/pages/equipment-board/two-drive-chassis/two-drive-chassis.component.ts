...
export class TwoDriveChassisComponent implements OnInit {
  ...
  resize = () => {
    setTimeout(() => {
      ...
      [
        ...
        "avl_param_chart_1",
        "avl_param_chart_2",
        "t_real_temperature_5",
        "t_real_temperature_4",
        "two_discharge_chart_1",
      ].forEach((f) => {
  ...
  get_real_time() {
    ...
    this.subscribeList.real = this.http
      .callRPC(
        ...
        { device: this.deviceid, arr: param.join(",") }
      )
      .subscribe((f: any) => {
  ...
  get_device_Temp_hum() {
    let res;
    this.subscribeList.temp_hum = this.http
      ...
      .subscribe((g: any) => {
        ...
        if (document.getElementById("t_real_temperature_5"))
          equipment_four_road.create_real_disk(
            {
              value: res.temperature,
              text: this.language ? "RealTEMP" : "实时温度",
              unit: "%RH",
            },
        ...
        if (document.getElementById("t_real_temperature_4"))
          equipment_four_road.create_real_disk(
            {
              value: res.humidity,
              text: this.language ? "RealRH" : "实时湿度",
              unit: "℃",
            },
  ...
  get_his_temp_hum() {
    let res;
    this.subscribeList.his_temp_hum = this.http
      .callRPC("get_temperature", library + "get_temperature_numbers", {
        deviceid: t_h_deviceid || this.deviceid,
      })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        ...
        if (document.getElementById("two_discharge_chart_1")) {
          let myChart_9 = echarts.init(
            document.getElementById("two_discharge_chart_1")
          );
          equipment_four_road.create_real_discharge(
            { attrs: this.temp_hum_attrs, xData: this.temp_hum_xData },
            myChart_9
          );
        }
      });
