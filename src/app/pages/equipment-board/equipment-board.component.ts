...
export class EquipmentBoardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subscribeList.resize = this.boradservice
      .chartResize()
      .subscribe((f) => {
        this.resize();
      });
    ...
    if (menu) {
      ...
      this.b_show.home = menu.find((f) => f.link && f.link == "/pages/home")
        ? true
        : false; //是否有主页权限
    }

    this.subscribeList.load = this.boradservice
      .get_Load_Observable()
      .subscribe((f) => {
        if (f.close) {
