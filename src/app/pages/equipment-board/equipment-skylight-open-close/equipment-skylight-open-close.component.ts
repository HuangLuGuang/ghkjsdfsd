...
export class EquipmentSkylightOpenCloseComponent implements OnInit {
  ...
  get_atec_temp() {
    ...
    this.http
      .callRPC(
        ...
        { device: this.atec_deviceid, arr: atec.join(",") }
      )
      .subscribe((g: any) => {
  ...
  get_sky() {
    ...
    this.http
      ...
      .subscribe((g: any) => {
        ...
        setTimeout(() => {
          this.ngzone.runOutsideAngular(() => {
            this.electric_temp.attrs[0].value = res[0].station1_motor_temp.map(
              (m) => m[0] || 0
            );
            this.electric_temp.attrs[1].value = res[1].station2_motor_temp.map(
              (m) => m[0] || 0
            );

            arr = res[0].station1_motor_temp;
            this.electric_temp.realdata_1 =
              arr.length > 0 ? arr[arr.length - 1][0] : 0;
            arr = res[1].station2_motor_temp;
            this.electric_temp.realdata_2 =
              arr.length > 0 ? arr[arr.length - 1][0] : 0;

            let i = 0,
              c = "station1_motor_temp";
            if (
              this.electric_temp.attrs[0].value.length <
              this.electric_temp.attrs[1].value.length
            ) {
              (i = 1), (c = "station2_motor_temp");
            }
            this.electric_temp.xdata = res[i][c].map((m) =>
              dateformat(new Date(rTime(m[1])), "hh:mm:ss")
            );
            if (document.getElementById("electric_temp")) {
              let myChart_9 = echarts.init(
                document.getElementById("electric_temp")
              );
              equipment_four_road.create_real_discharge(
                {
                  attrs: this.electric_temp.attrs,
                  xData: this.electric_temp.xdata,
                },
                myChart_9
              );
            }
  ...
  get_log() {
    let res;
    this.http
      .callRPC("get_log", library + "get_logs", { deviceid: this.deviceid })
      .subscribe((g: any) => {
        if (g.result.error || g.result.message[0].code == 0) return;
        res = g.result.message[0].message;
        this.table_1 = res[0].map((m) => [
          dateformat(new Date(m.recordtime), "hh:mm:ss"),
          this.log_lv(m.level),
          m.message,
        ]);
      });
  ...
  initChart() {
    ...
    if (document.getElementById("electric_temp")) {
      ...
      equipment_four_road.create_real_discharge(
        { attrs: this.electric_temp.attrs, xData: this.electric_temp.xdata },
        myChart_9
      );
    }
