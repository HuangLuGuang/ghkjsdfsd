...
export class EimFileUploadComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.del_file_col = {
      ...
      cellRendererParams: {
        clicked: function (data: any) {
          // 点击删除时，调用
          const ref = that.publicservice.showConfirm('提示', `确认要删除${data['filename']}吗？`);
  ...
  onCreateFolder() {
    this.dialogService.open(InputFoldernameComponent).onClose.subscribe(name => {
  ...
  get_current_path_files() {
    ...
    this.http.post<any>('/api/v1/get_files', body, this.hearder).subscribe(result => {
