...
export class DeviceAndonHistoryComponent implements OnInit {
  ...
  tableDatas = {
    ...
    columnDefs: [
      ...
      },
      {
        field: "errmsg",
        headerName: "故障描述",
        cellRendererFramework: ErrmsgComponent,
        minWidth: 10,
        resizable: true,
        sortable: true,
        flex: 1,
  ...
  ngOnInit(): void {
    ...
    this.device_andon_history_status = JSON.parse(
      localStorage.getItem("device_andon_history_status")
    );
    ...
    this.http.callRPC(table, method, colums).subscribe((result) => {
      ...
      if (res["code"] === 1) {
        ...
        this.RecordOperation(
          "搜索",
          1,
          "设备安灯历史状态:" + JSON.stringify(colums)
        );
      } else {
        this.RecordOperation(
          "搜索",
          0,
          "设备安灯历史状态:" + JSON.stringify(res["message"])
        );
