...
export class DeviceManageComponent implements OnInit {
  ...
  edit(active_data?) {
    ...
    if (rowdata.length === 0) {
      ...
      this.dialogService
        .open(EditDelTooltipComponent, {
          context: { title: "编辑设备提示", content: `请选择需要编辑的行！` },
        })
        .onClose.subscribe((name) => {
          console.log("----name-----", name);
        });
      ...
      this.dialogService
        .open(Add_Edit_DeviceManageComponent, {
          ...
          context: {
            title: "编辑设备提示",
            content: `true`,
            rowData: JSON.stringify(rowdata_),
          },
        })
        .onClose.subscribe((name) => {
          console.log("----name-----", name);
  ...
  query(inpuvalue?) {
    ...
    if (devicename == "" && eimdevicetype.length < 1 && grops_data.length < 1) {
      ...
      this.http
        .callRPC("device", "dev_get_device_search", columns)
        .subscribe((result) => {
  ...
  tableDatas = {
    ...
    columnDefs: [
      ...
      },
      {
        ...
        resizable: true,
        minWidth: 10,
        cellRendererFramework: TranIscalkpiComponent,
        sortable: true,
      },
    ],
    rowData: [
