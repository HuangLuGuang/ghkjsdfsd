...
export class GroupDevicesComponent implements OnInit {
  @Output() device_info = new EventEmitter<any>(); // 这是选择设备名称触发
  // 科室/功能组下拉框
  groups = [];
  // 设备id
  deviceid;
  ...
  layuiform() {
    ...
    layui.use("form", function () {
      ...
      form.on("select(test_task_conf_add_devicename)", function (data) {
        ...
        that.update_deviceno(data.value, true);
      });
    });
  }
      ...
      if (res["code"] === 1) {
        ...
        devicename_deviceno.forEach((element, index) => {
          if (index === 0) {
            ...
            this.deviceid = element.deviceno;
            this.deviceno = element.deviceid;
            this.device_info.emit(this.get_form_val());
  ...
  // 更改设备编号 deveceno
  update_deviceno(deviceno, istrue?) {
