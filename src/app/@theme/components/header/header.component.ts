...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.userService
      ...
      .subscribe((users: any) => {
        ...
        var token = this.localStorageService.get(ssotoken)
          ? this.localStorageService.get(ssotoken)
          : false;
  ...
  toggleSidebar(): boolean {
    var echarts_resize =
      localStorage.getItem(ECHARTS_RESIZE) === null
        ? false
        : JSON.parse(localStorage.getItem(ECHARTS_RESIZE));
  ...
  alet_is_logout(title) {
    ...
    if (title == this.userMenu[0]["title"]) {
      ...
      var ticket = this.localStorageService.get(ssotoken)
        ? this.localStorageService.get(ssotoken)
        : false;
  ...
  RecordLogin() {
    ...
    console.log(
      "this.userinfoservice.getLoginName()",
      this.userinfoservice.getLoginName()
    );
  }
}
